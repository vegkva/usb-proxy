...
void usage() {
  ...
  printf(
      "\t--enable_customized_config: enable the customized config feature\n\n");
  printf("\t--command_injection: enable command injection\n\n");
  printf("* If `device` not specified, `usb-proxy` will use `dummy_udc.0` as "
         "default device.\n");
  printf("* If `driver` not specified, `usb-proxy` will use `dummy_udc` as "
         "default driver.\n");
  printf("* If both `vendor_id` and `product_id` not specified, `usb-proxy` "
         "will connect\n");
  printf("  the first USB device it can find.\n");
  printf("* If `injection_file` not specified, `usb-proxy` will use "
         "`injection.json` by default.\n\n");
  exit(1);
}
    ...
    for (int j = 0; j < bNumInterfaces; j++) {
      ...
      for (int k = 0; k < num_altsetting; k++) {
        ...
        if (!temp_device_altsetting.bNumEndpoints) {
          printf(
              "InterfaceNumber %x AlternateSetting %x has no endpoint, skip\n",
        ...
        for (int l = 0; l < bNumEndpoints; l++) {
          struct usb_endpoint_descriptor temp_endpoint = {
              ...
              .bDescriptorType =
                  temp_device_altsetting.endpoint[l].bDescriptorType,
              .bEndpointAddress =
                  temp_device_altsetting.endpoint[l].bEndpointAddress,
              .bmAttributes = temp_device_altsetting.endpoint[l].bmAttributes,
              .wMaxPacketSize =
                  temp_device_altsetting.endpoint[l].wMaxPacketSize,
      ...
      temp_interfaces[j].num_altsettings =
          device_config_desc[i]->interface[j].num_altsetting;
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < bNumConfigurations; i++) {
    ...
    for (int j = 0; j < bNumInterfaces; j++) {
      int num_altsetting = device_config_desc[i]->interface[j].num_altsetting;
      for (int k = 0; k < num_altsetting; k++) {
        delete[] host_device_desc.configs[i]
            .interfaces[j]
            .altsettings[k]
            .endpoints;
      }
  ...
  if (hotplug_monitor_thread && pthread_join(hotplug_monitor_thread, NULL)) {
